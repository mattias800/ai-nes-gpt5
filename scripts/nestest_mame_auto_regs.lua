-- scripts/nestest_mame_auto_regs.lua
-- Run nestest in auto mode (PC=$C000, A=0, X=0, Y=0, P=$24, S=$FD) and dump per-instruction regs.
-- Env:
...
if not dbg then emu.print_error('[nestest-auto-regs] debugger not available; run with -debug'); return end
local cpu = manager.machine.devices[':maincpu'] or manager.machine.devices['maincpu']
if not cpu or not cpu.state then emu.print_error('[nestest-auto-regs] maincpu state not available'); return end
local st = cpu.state
...
if not fh then emu.print_error('[nestest-auto-regs] cannot open output file: ' .. out); return end
emu.print_info(string.format('[nestest-auto-regs] tracing %d instructions to %s', inst, out))
...
fh:flush(); fh:close()
emu.print_info('[nestest-auto-regs] complete; exiting')
