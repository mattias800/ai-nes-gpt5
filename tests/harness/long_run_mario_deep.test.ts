...
describe.skipIf(!findLocalRom() || !LONG_RUN)('Long-run Mario integration (deep, optional)', () => {
  it('runs ~600 frames and computes a stable state CRC', { timeout: vitestTimeout('HARNESS_WALL_TIMEOUT_MS', 600000) }, () => {
    ...
    // Minimal PPU enable for visuals
    ...
    const palette = (sys.ppu as any)['palette'] as Uint8Array;
    ...
    const baseline = process.env.LONG_RUN_DEEP_BASELINE_CRC;
    if (baseline) {
      ...
      console.log(`Deep long-run state CRC32: 0x${hash.toString(16).toUpperCase().padStart(8,'0')} (${hash})`);
