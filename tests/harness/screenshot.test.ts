import { describe, it, expect } from 'vitest';
import fs from 'node:fs';
import path from 'node:path';
import { NESSystem } from '@core/system/system';
import { parseINes } from '@core/cart/ines';
import { PNG } from 'pngjs';

(function loadDotEnv(){
  try {
    const p = path.resolve('.env');
    if (fs.existsSync(p)) {
      const t = fs.readFileSync(p, 'utf-8');
      for (const raw of t.split(/\r?\n/)) {
        const line = raw.trim(); if (!line || line.startsWith('#')) continue;
        const i = line.indexOf('='); if (i <= 0) continue;
        const k = line.slice(0, i).trim(); let v = line.slice(i+1).trim();
        if ((v.startsWith('"') && v.endsWith('"')) || (v.startsWith('\'') && v.endsWith('\''))) v = v.slice(1, -1);
        if (!(k in process.env)) process.env[k] = v;
      }
    }
  } catch {}
})();

function findLocalRom(): string | null {
  const cwd = process.cwd();
  const files = fs.readdirSync(cwd).filter((f) => f.toLowerCase().endsWith('.nes'));
  if (files.length === 0) return null;
  files.sort((a, b) => {
    const am = a.toLowerCase().startsWith('mario') ? 0 : 1;
    const bm = b.toLowerCase().startsWith('mario') ? 0 : 1;
    return am - bm;
  });
  return path.join(cwd, files[0]);
}

function nesPaletteToRGB(idx: number): [number, number, number] {
  // Simple NTSC NES palette approximation (subset). For testing/screenshot we can use a compact table.
  // This is a minimal mapping; you can replace with a more accurate palette later.
  const PALETTE: [number, number, number][] = [
    [84,84,84],[0,30,116],[8,16,144],[48,0,136],[68,0,100],[92,0,48],[84,4,0],[60,24,0],
    [32,42,0],[8,58,0],[0,64,0],[0,60,0],[0,50,60],[0,0,0],[0,0,0],[0,0,0],
    [152,150,152],[8,76,196],[48,50,236],[92,30,228],[136,20,176],[160,20,100],[152,34,32],[120,60,0],
    [84,90,0],[40,114,0],[8,124,0],[0,118,40],[0,102,120],[0,0,0],[0,0,0],[0,0,0],
    [236,238,236],[76,154,236],[120,124,236],[176,98,236],[228,84,236],[236,88,180],[236,106,100],[212,136,32],
    [160,170,0],[116,196,0],[76,208,32],[56,204,108],[56,180,204],[60,60,60],[0,0,0],[0,0,0],
    [236,238,236],[168,204,236],[188,188,236],[212,178,236],[236,174,236],[236,174,212],[236,180,176],[228,196,144],
    [204,210,120],[180,222,120],[168,226,144],[152,226,180],[160,214,228],[160,162,160],[0,0,0],[0,0,0],
  ];
  return PALETTE[(idx & 0x3F) % PALETTE.length];
}

async function writePng(pathOut: string, fb: Uint8Array, w = 256, h = 240) {
  const png = new PNG({ width: w, height: h });
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const i = y * w + x;
      const [r,g,b] = nesPaletteToRGB(fb[i] & 0x3F);
      const o = (y * w + x) << 2;
      png.data[o+0] = r;
      png.data[o+1] = g;
      png.data[o+2] = b;
      png.data[o+3] = 255;
    }
  }
  const dir = path.dirname(pathOut);
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  const stream = fs.createWriteStream(pathOut);
  await new Promise<void>((resolve, reject) => {
    stream.on('finish', () => resolve());
    stream.on('error', (e) => reject(e));
    png.pack().pipe(stream);
  });
}

describe.skipIf(!findLocalRom())('Screenshot harness (optional)', () => {
  it('renders background-only and full-frame PNGs after N frames', async () => {
    const romPath = findLocalRom()!;
    const rom = parseINes(new Uint8Array(fs.readFileSync(romPath)));
    const sys = new NESSystem(rom);
    sys.reset();

    // Enable background + sprites for full frame; and BG for bg-only shot
    sys.io.write(0x2001, 0x1E);

    // Run some frames to reach a stable screen
    const frames = parseInt(process.env.SCREENSHOT_FRAMES || '60', 10);
    const start = sys.ppu.frame;
    while (sys.ppu.frame < start + frames) sys.stepInstruction();

    // Background-only
    const bg = (sys.ppu as any).renderBgFrame() as Uint8Array;
    await writePng('screenshots/mario_bg.png', bg);

    // Full frame (bg + sprites)
    const full = (sys.ppu as any).renderFrame() as Uint8Array;
    await writePng('screenshots/mario_full.png', full);

    expect(fs.existsSync('screenshots/mario_bg.png')).toBe(true);
    expect(fs.existsSync('screenshots/mario_full.png')).toBe(true);
  });
});

