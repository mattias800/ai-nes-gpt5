...
describe.skipIf(!findLocalRom())('Framebuffer hash smoke (bg+sprites)', () => {
  it('runs a few frames then hashes full framebuffer', { timeout: vitestTimeout('HARNESS_WALL_TIMEOUT_MS', 120000) }, () => {
    ...
    const fb = (sys.ppu as any).renderFrame();
    const hash = crc32(fb);

    const baseline = process.env.FB_BASELINE_FULL;
    if (baseline) {
      ...
      console.log(`Full framebuffer CRC32: 0x${hash.toString(16).toUpperCase().padStart(8,'0')} (${hash})`);
