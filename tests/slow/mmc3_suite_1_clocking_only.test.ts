/*
Run only 1-clocking.nes from the MMC3 test suite and print detailed diagnostics on failure.
*/
...
describe('MMC3: 1-clocking.nes only', () => {
  it('should pass', () => {
    ...
    if (res.status !== 0) {
      ...
      console.error(`[1-clocking DEBUG] status=${res.status} msg=${JSON.stringify(res.message)} cycles=${res.cycles}`);
      // eslint-disable-next-line no-console
      console.error(`[1-clocking DEBUG] A12 rises (last ${a12.length}): ${a12.map(t => `f${t.frame}s${t.scanline}c${t.cycle}`).join(', ')}`);
      const ctrl = (res as any).ctrlTrace ?? [];
      const mask = (res as any).maskTrace ?? [];
      if (ctrl && ctrl.length) {
        const tailc = ctrl.slice(-32);
        // eslint-disable-next-line no-console
        console.error(`[1-clocking DEBUG] PPUCTRL writes (last ${tailc.length}/${ctrl.length}): ${tailc.map((e: any)=>`@[f${e.frame}s${e.scanline}c${e.cycle}] $2000=${(e.ctrl>>>0).toString(16).padStart(2,'0')}`).join(', ')}`);
      }
      if (mask && mask.length) {
        const tailm = mask.slice(-32);
        // eslint-disable-next-line no-console
        console.error(`[1-clocking DEBUG] PPUMASK writes (last ${tailm.length}/${mask.length}): ${tailm.map((e: any)=>`@[f${e.frame}s${e.scanline}c${e.cycle}] $2001=${(e.mask>>>0).toString(16).padStart(2,'0')}`).join(', ')}`);
      }
      if (mmc3 && (mmc3 as any).length) {
        ...
        if (phase && phase.length) {
          ...
          // eslint-disable-next-line no-console
          console.error(`[1-clocking DEBUG] PPU phase trace (last ${tailp.length}): ${tailp.map((e: any)=>`@[f${e.frame}s${e.scanline}c${e.cycle}] ctrl=${(e.ctrl>>>0).toString(16).padStart(2,'0')} mask=${(e.mask>>>0).toString(16).padStart(2,'0')}${e.emitted!==undefined?` emitted=${e.emitted}`:''}`).join(' | ')}`);
        }
        ...
        // eslint-disable-next-line no-console
        console.error(`[1-clocking DEBUG] MMC3 trace counts: ${Object.entries(counts).map(([k,v])=>`${k}:${v}`).join(', ')}`);
        const tail = arr.slice(Math.max(0, arr.length - 32));
        // eslint-disable-next-line no-console
        console.error(`[1-clocking DEBUG] MMC3 trace (last ${tail.length}): ${tail.map((e: any) => e.type + (e.ctr!==undefined?`(ctr=${e.ctr}${e.en!==undefined?`,en=${e.en}`:''})`:'' ) + (e.v!==undefined?` v=${e.v}`:'' ) + (e.a!==undefined?` a=${e.a}`:'' ) + (e.f!==undefined?` @[f${e.f}s${e.s}c${e.c}]`:'' )).join(' | ')}`);
      }
    }
